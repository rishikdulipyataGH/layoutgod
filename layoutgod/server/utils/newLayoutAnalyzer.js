...
class LayoutAnalyzer {
  ...
  loadBigramFreq() {
    try {
      const bigramPath = path.join(__dirname, '../../cyanophage.github.io-main/bigram_effort.json');
      if (fs.existsSync(bigramPath)) {
        return JSON.parse(fs.readFileSync(bigramPath, 'utf8'));
      }
    } catch (error) {
      console.warn('Could not load bigram frequencies:', error.message);
    }
    return {};
  }
